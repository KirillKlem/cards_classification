...
### Other details
All training takes place in google colab using the gpu. Various architectures were tried at an early stage, as well as other bases for the model (for example, Swin, Vit, EfficientNet). Loss and accuracy are visualized for tracking on train and validation data. It is planned to use TensorBoard for this. Temporarily, most of the code is in a single file (main.ipynb), while the project does not need a complex structure, however, it is planned to break everything into smaller parts.
